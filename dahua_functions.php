   ...
   function Main($reconnectTimeout=60)
    {
        ...
        while (true){
            ...
            if (!count($data) || !array_key_exists('result', json_decode($data[0], true))){
                echo "Failure eventManager.attach";
                continue;
            }
            else{
                unset($data[0]);
	...
	if(count($data['params']['eventList'])>1){
		echo "Event Manager subscription reply... ";
	}
...
//on verifie dans le script qu'il y'a bien une zone definie dans les regles de videomotion (zone est dans le nom)
///if (array_key_exists('Name', $eventData)) {
		logging($eventCode.' '.$eventData['Name'].' '.$eventData['Action']);
///}else { echo 'Videomotion not send - no area defined'; }
//var_dump($data);
	//	$this->SaveSnapshot();
	}
...
//CrossLineDetection
	elseif($eventCode == 'CrossLineDetection'){
                logging("Crossline detection ".$eventList['Action']. ' Zone '.$eventData['Name']);
        }
   elseif($eventCode == 'SmartMotionHuman'){
                logging("SmartMotionHuman ".$eventList['Action'].", LocaleTime ".$eventData['LocaleTime']);
        } 
   elseif($eventCode == 'ParkingDetection'){
               // logging("ParkingDetection ".$eventList['Action'].", LocaleTime ".$eventData['LocaleTime']);
logging("ParkingDetection ".$eventList['Action'].' Zone '.$eventData['Name']);
        } 
	else{
	//	logging($eventList['Code'] . ' ' . $eventList['Action']);
        echo 'not send data jeedom......';	
	if($debug) var_dump($data);

$cheminFichierLog = '/var/log/dahua.log';

// Format des données à enregistrer dans le fichier
$logData = date('Y-m-d H:i:s') . ' - ' . $eventCode . ' ' . $eventAction . ' - ' . print_r($data, true) . PHP_EOL;

// Ajouter les données dans le fichier
file_put_contents($cheminFichierLog, $logData, FILE_APPEND);
